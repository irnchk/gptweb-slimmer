...
![한국어](https://img.shields.io/badge/%ED%95%9C%EA%B5%AD%EC%96%B4-2563eb?style=for-the-badge)
...
- 팝업에서 언어 설정을 바꿔 확장 UI를 한국어와 영어 사이에서 전환할 수 있습니다.
- 우하단 도크나 펼친 패널 상단을 드래그해서 페이지 안 UI 위치를 옮길 수 있고, 위치도 브라우저별로 로컬 저장됩니다.
...
- 확장 아이콘을 눌러 항상 남길 턴 수, `이전 더 보기` 배치 크기, UI 언어를 실시간으로 조절합니다.
- 기본값은 최근 40개 턴 유지입니다.
- 오래된 턴을 다시 보고 싶으면 페이지 상단 자리표시자, 접힌 도크를 펼친 패널, 또는 팝업에서 `이전 더 보기`를 누릅니다.
- 입력창이나 다른 페이지 요소를 가린다면 우하단 도크 또는 패널 상단을 드래그해서 원하는 위치로 옮길 수 있습니다.
